...
class ultimateTicTacToe:
    ...
    def playGamePredifinedAgent(self, maxFirst, isMinimaxOffensive, isMinimaxDefensive):
        ...
        while self.checkMovesLeft() and not self.checkWinner():
            ...
            for i in range(9):
                ...
                # self.expandedNodes+=1
                val = self.minimax(1, i, not self.currPlayer) if (isMinimaxOffensive and self.currPlayer) or (
                            isMinimaxDefensive and not self.currPlayer) else self.alphabeta(1, i, -inf, inf,
                                                                                            not self.currPlayer)
                self.board[row][col] = '_'  # remove it for other tests
    ...
    def playGameYourAgent(self):
        ...
        self.currPlayer = True if randint(0, 1) else False
    ...
    def playGameHuman(self):
        ...
        atglobal = randint(0, 8)  # stores which global index we are at
        self.currPlayer = True if randint(0,
                                          1) else False  # This will imply that the predefined agent is going first (offensive)
        ...
        while (self.checkMovesLeft() and not self.checkWinner()):
            ...
            if not self.currPlayer:
                ...
                bestidx = min(
                    options[chosenval])  # this is just if multiple indices had same value, chose smallest index
                best = (self.globalIdx[atglobal][0] + bestidx // 3, self.globalIdx[atglobal][1] + bestidx % 3)
                bestMove.append(best)
